...
public class APrioriAlgorithm {
	...
	public APrioriAlgorithm(double minSupportLevel, double minConfidenceLevel, String filepath) {
		...
		this.minSupportLevel = (double)parser.getNumTransactions() * minSupportLevel;
	 ...
	 * GETTERS & SETTERS *
	 *********************/
	// Only the getters/setters that are actually used are written.
		...
		for (ItemSet currentItemSet : itemSubsets) {
			for (ItemSet comparisonItemSet : itemSubsets) {
				...
				if (confidenceLevel >= minConfidenceLevel
					...
					AssociationRule rule = new AssociationRule(currentItemSet, comparisonItemSet, (double)Math.round(confidenceLevel*100)/100);
